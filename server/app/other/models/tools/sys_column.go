...
type SysTableColumn struct {
	...
	FkCol              []SysTableColumn `gorm:"-" json:"fkCol"`
...
func (e *SysTableColumn) GetList(tx *gorm.DB, exclude bool) ([]SysTableColumn, error) {
	var doc []SysTableColumn
	table := tx.Table("sys_table_column")
...
func (e *SysTableColumn) Create(tx *gorm.DB) (SysTableColumn, error) {
	var doc SysTableColumn
	e.CreateBy = 0
	result := tx.Table("sys_table_column").Create(&e)
	if result.Error != nil {
		err := result.Error
		return doc, err
	}
...
func (e *SysTableColumn) Update(tx *gorm.DB) (update SysTableColumn, err error) {
	if err = tx.Table("sys_table_column").First(&update, e.ColumnId).Error; err != nil {
		return
	}
	...
	//参数2:是修改的数据
	e.UpdateBy = 0
	if err = tx.Table("sys_table_column").Model(&update).Updates(&e).Error; err != nil {
		return
	}
